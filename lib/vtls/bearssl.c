  ...
  for(;;) {
    ...
    while(n) {
      ...
      case BR_PEM_BEGIN_OBJ:
        ...
        if(strcmp(name, "CERTIFICATE") != 0 && strcmp(name, "X509 CERTIFICATE") != 0)
          break;
...
static CURLcode bearssl_connect_step1(struct connectdata *conn, int sockindex)
{
  ...
  BACKEND->anchors = NULL;
  BACKEND->anchors_len = 0;
  if(ssl_cafile) {
  ...
  br_ssl_client_init_full(&BACKEND->ctx, &BACKEND->x509, BACKEND->anchors, BACKEND->anchors_len);
